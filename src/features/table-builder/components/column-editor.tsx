...
interface ColumnEditorSheetProps {
	...
	setColumns: (
		columns: ColumnConfig[] | ((prev: ColumnConfig[]) => ColumnConfig[]),
	) => void;
					...
					<div className="space-y-3 pt-2 border-t">
						<div className="flex items-center space-x-2">
							<Checkbox
								...
								onCheckedChange={(checked) =>
									onToggleFacetedFilter(column.id, checked === true)
								}
							/>
							<Label
								htmlFor={`faceted-${column.id}`}
								...
								<div className="flex items-center justify-between">
									...
									<Select
										...
										onValueChange={(value: "auto" | "custom") =>
											onUpdateOptionsMode(column.id, value)
										}
									>
									...
									<div className="space-y-2">
										<div className="flex items-center justify-between">
											...
											<Button
												variant="outline"
												size="sm"
												onClick={() => onAddCustomOption(column.id)}
											>
											...
											{(column.options || []).map((option, optionIndex) => (
												...
												>
													...
													<Button
														...
														size="sm"
														onClick={() =>
															onRemoveCustomOption(column.id, optionIndex)
														}
													>
														<X className="h-3 w-3" />
													</Button>
					...
					<div className="space-y-3 pt-2 border-t">
						<div className="flex items-center space-x-2">
							<Checkbox
								...
								onCheckedChange={(checked) =>
									onToggleDateFilter(column.id, checked === true)
								}
							/>
							<Label
								htmlFor={`datefilter-${column.id}`}
								className="text-sm font-medium"
							>
								Enable Date Filter
							</Label>
						...
						{column.hasDateFilter && (
							<div className="ml-6 text-xs text-muted-foreground">
								Users can filter this column by selecting a date range with
								start and end dates.
							</div>
						)}
					</div>
					...
					<div className="space-y-3 pt-2 border-t">
						<div className="flex items-center space-x-2">
							<Checkbox
								...
								onCheckedChange={(checked) =>
									onToggleSliderFilter(column.id, checked === true)
								}
							/>
							<Label
								htmlFor={`sliderfilter-${column.id}`}
								className="text-sm font-medium"
							>
						...
						{column.hasSliderFilter && (
							<div className="ml-6 text-xs text-muted-foreground">
								Users can filter this column using a range slider with min/max
								values.
							</div>
						)}
					</div>
...
export function ColumnEditorSheet({
	...
	const toggleDateFilter = (columnId: string, enabled: boolean) => {
		setColumns((prev) =>
			prev.map((col) =>
				col.id === columnId ? { ...col, hasDateFilter: enabled } : col,
			),
		);
	};
	...
	const toggleSliderFilter = (columnId: string, enabled: boolean) => {
		setColumns((prev) =>
			prev.map((col) =>
				col.id === columnId ? { ...col, hasSliderFilter: enabled } : col,
			),
		);
	};
	...
	const removeCustomOption = (columnId: string, optionIndex: number) => {
		setColumns((prev) =>
			prev.map((col) => {
				if (col.id === columnId && col.options) {
					const newOptions = col.options.filter(
