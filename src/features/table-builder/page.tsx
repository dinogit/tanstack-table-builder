...
import {CheckCircle, FileText, Plus, Settings, X} from "lucide-react";
...
import {
    arrayContainsFilter,
    booleanFilter, dateRangeFilter, multiSelectFilter,
    numberFilter,
    selectFilter,
    textFilter
...
export function Page() {
    ...
    const tableColumns = React.useMemo<ColumnDef<JsonData>[]>(() => {
        ...
        const dataColumns = allColumns.map((column) => ({
            ...
            // filterFn:
            //     column.hasFacetedFilter && column.type === "string"
            //         ? "faceted"
            //         : column.type === "date"
            //             ? "dateFilter"
            //             : column.hasSliderFilter && column.type === "number"
            //                 ? "sliderFilter"
            //                 : undefined,
            filterFn:
                column.hasFacetedFilter && column.type === "string"
                    ? "multiSelect" // use lib fn name
                    : column.type === "date"
                        ? "dateRangeFilter"
                        : column.hasSliderFilter && column.type === "number"
                            ? "number" // slider = numeric range
                            : undefined,
            cell: ({ getValue }) => {
    ...
    const table = useReactTable({
        ...
        // filterFns: {
        //     faceted: (row, columnId, filterValue) => {
        ...
        //             const [fromTimestamp, toTimestamp] = filterValue
        ...
        //         }
        //     },
        //     sliderFilter: (row, columnId, filterValue) => {
        //         const cellValue = row.getValue(columnId)
        //         if (!filterValue || !Array.isArray(filterValue)) return true
        //
        //         const numericValue = typeof cellValue === "number" ? cellValue : Number.parseFloat(String(cellValue))
    ...
    const facetedFilterColumns = React.useMemo(() => {
        return columns
            .filter((col) => columnVisibility[col.id] && col.hasFacetedFilter && col.type === "string")
            .map((col) => ({
                ...col,
    ...
    const dateFilterColumns = React.useMemo(() => {
        return columns
            .filter((col) => columnVisibility[col.id] && col.type === "date" && col.hasDateFilter)
            .slice(0, 2)
    }, [columns, columnVisibility])

    const sliderFilterColumns = React.useMemo(() => {
        return columns
            .filter((col) => columnVisibility[col.id] && col.type === "number" && col.hasSliderFilter)
            .slice(0, 3)
    }, [columns, columnVisibility])
    ...
    const hasActiveFilters = () => {
        return globalFilter !== "" || columnFilters.length > 0 || sorting.length > 0
    }
        ...
        <div className="min-h-screen bg-background p-4">
            <div className="max-w-full mx-auto space-y-6">
                ...
                <div className="text-center space-y-2">
                    <h1 className="text-3xl font-bold tracking-tight">Tanstack Table Builder</h1>
                    <p className="text-muted-foreground">Transform your JSON data into beautiful, exportable React tables</p>
                </div>
                    ...
                    <CardHeader>
                        <div className="flex items-center justify-between">
                            ...
                            <div className="flex items-center gap-2">
                                <Button variant="outline" size="sm" onClick={() => setDataInputOpen(true)}>
                                ...
                                <Button variant="outline" size="sm" disabled={data.length === 0} onClick={() => setToggleCopyCodeSheet(true)}>
                                    Copy Code
                                </Button>
                            </div>
                        </div>
                        ...
                        {data.length === 0 ? (
                            <div className="flex flex-col items-center justify-center py-12 text-center">
                                ...
                                <p className="text-muted-foreground">
                                    Add your JSON data using the "Add Data" button above to see the table preview and configure columns.
                                </p>
                            </div>
                        ) : (
                                ...
                                <div className="flex items-center justify-between">
                                    <div className="flex items-center space-x-2">
                                        ...
                                        {sliderFilterColumns.map((column) => (
                                            <DataTableSliderFilter key={column.id} column={table.getColumn(column.id)} title={column.label} />
                                        ))}
                                        {hasActiveFilters() && (
