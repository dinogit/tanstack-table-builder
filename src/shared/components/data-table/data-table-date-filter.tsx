...
export function DataTableDateFilter({ column, title, multiple }: DataTableDateFilterProps) {
    ...
    <Popover>
      <PopoverTrigger asChild>
        <Button variant="outline" size="sm" className="h-8 border-dashed bg-transparent">
          {hasValue ? (
            <XCircle className="mr-2 h-4 w-4 cursor-pointer" onClick={onReset} />
          ) : (
      ...
      <PopoverContent className="w-auto p-0" align="start">
        {multiple ? (
          ...
          <Calendar
            mode="single"
            selected={Array.isArray(selectedDates) ? selectedDates[0] : undefined}
            onSelect={onSelect}
            initialFocus
          />
        )}
